...
void drawBird(){
	...
	}SetConsoleTextAttribute(console,2);//Yellow2
...
void play(){
	...
	gotoxy(WIN_WIDTH + 6, 7);cout<<"Highscore:";
	gotoxy(WIN_WIDTH + 6, 8);cout<<highscore;
	...
	while(1){
		...
		if(kbhit()){
			...
			if(ch==32){
				if( birdPos > 3 )
					birdPos-=3;
				mciSendString("play wing.wav", NULL, 0, NULL);
			} 
			if(ch==27){
				SetConsoleTextAttribute(console,15);
				break;
		...
		if( birdPos > SCREEN_HEIGHT - 2 ){
			...
			PlaySound("fall.wav",NULL,SND_ASYNC);
			sleep(1);
			gameover();
			return;
		}
		...
		if( pipePos[0] > 68 ){
			mciSendString("play point.wav", NULL, 0, NULL);
			score+=100;
			updateScore();
			pipeFlag[1] = 0;
			pipePos[0] = pipePos[1];
...
//	play();
	...
	do{
		...
		gotoxy(10,13); cout<<"Highscore : "<< highscore;
		
		gotoxy(10,15); cout<<"Select Menu : ";
		char op = getche();
